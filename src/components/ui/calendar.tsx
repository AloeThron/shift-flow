...
/** dropdown เดือน/ปี — ใช้ shadcn Select แทน native select */
function CalendarSelectDropdown({
  ...
  /** แปลง onValueChange ให้เข้ากับ API ของ react-day-picker */
  ...
  return (
    <Select value={selectedValue} onValueChange={handleValueChange} disabled={disabled}>
      <SelectTrigger
        aria-label={ariaLabel}
        size="sm"
        className={cn("min-w-[6.5rem] rounded-xl", className)}
      >
        <SelectValue />
      </SelectTrigger>
      <SelectContent align="center" className="z-[200] max-h-60">
        <SelectGroup>
          {options?.map((option) => (
            <SelectItem
              key={option.value}
    ...
    <DayPicker
      ...
      formatters={{
        formatMonthDropdown: (date) =>
          date.toLocaleString(locale?.code ?? "th-TH", { month: "short" }),
        ...formatters,
      }}
      classNames={{
        ...
        dropdowns: cn("flex w-full items-center justify-center gap-2", defaultClassNames.dropdowns),
